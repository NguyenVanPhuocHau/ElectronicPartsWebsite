...
//        System.out.println(user.getUser_id() + " " + name + " " +  address +" " + phone + " " + email + " " + cart.getFinalMoneyCart());
        String idUser = "";
        if (user == null){
            idUser = "US" + (random.nextInt(9999) + 1000);
        }
        else {
            idUser = user.getUser_id();
        }
        OrderDao.getInstance().insertOrder(orderId,idUser,name,address,phone,email,"ST001",cart.getFinalMoneyCart(),new Date(2022-12-19));
